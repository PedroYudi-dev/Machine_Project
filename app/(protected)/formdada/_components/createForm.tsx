  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          Preencha os dados do cliente para prever o risco de cancelamento
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form
        ...
        >
          ...
          <div className="flex flex-col gap-4">
            ...
            />
            <RadioField
              label="Partner (Possui Parceiro)?"
              name="Partner"
              control={control}
              options={simNao}
            />
            <RadioField
              label="Dependents (Possui Dependentes)?"
              name="Dependents"
              control={control}
              options={simNao}
            />
          ...
          <div className="flex flex-col gap-4">
            ...
            <RadioField
              label="Multiple Lines (Múltiplas Linhas)?"
              name="MultipleLines"
              control={control}
              options={[
                { label: "Sim", value: "Yes" },
                { label: "Não", value: "No" },
                { label: "Sem telefone", value: "No phone service" },
              ]}
            />
            <RadioField
              label="Internet Service (Serviço de Internet)?"
            ...
            />
            <RadioField
              label="Online Security (Segurança Online)?"
              name="OnlineSecurity"
              control={control}
              options={simNao}
            />
            <RadioField
              label="Online Backup (Backup Online)?"
              name="OnlineBackup"
              control={control}
              options={simNao}
            />
            <RadioField
              label="Device Protection (Proteção de Dispositivo)?"
              name="DeviceProtection"
              control={control}
              options={simNao}
            />
            <RadioField
              label="Tech Support (Suporte Técnico)?"
              name="TechSupport"
              control={control}
              options={simNao}
            />
            ...
            />
            <RadioField
              label="Streaming Movies (Streaming Filmes)?"
          ...
          <div className="flex flex-col gap-4">
            ...
            <RadioField
              label="Contract (Tipo de Contrato)?"
              name="Contract"
              control={control}
              options={[
                { label: "Mensal", value: "Month-to-month" },
                { label: "1 Ano", value: "One year" },
                { label: "2 Anos", value: "Two year" },
              ]}
            />
            <RadioField
              label="Paperless Billing (Fatura Digital)?"
            ...
            <div className="flex flex-col gap-1">
              <Label>Mensalidade (MonthlyCharges)</Label>
              <Input
                type="number"
                step="0.01"
            ...
            <div className="flex flex-col gap-1">
              <Label>Total Pago (TotalCharges)</Label>
              <Input
                type="number"
                step="0.01"
            ...
            <div className="flex flex-col gap-1">
              <Label>Tempo como cliente (meses) (tenure)</Label>
              <Input
                type="number"
                placeholder="Ex: 12"
                {...register("tenure")}
          ...
          {resultado && (
            <div
              className={`rounded-lg p-4 flex flex-col gap-2 ${resultado.risco === "Alto" ? "bg-red-50 border border-red-200" : "bg-green-50 border border-green-200"}`}
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold">
                  {(resultado.probabilidade_churn * 100).toFixed(0)}%
                </span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Badge
                  variant={
                    resultado.risco === "Alto" ? "destructive" : "default"
                  }
                >
                  {resultado.risco}
                </Badge>
